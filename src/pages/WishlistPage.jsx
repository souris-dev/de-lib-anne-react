...
export function WishlistPage() {
  ...
  const [wishlistIsEmpty, setWishlistIsEmpty] = useState(false);
  const { username: retrieveUsername } = useContext(LoginContext);

  useEffect(() => {
    getData(atServiceEndpoint("wishlist", "/getwishlist"))
      .then((response) => {
        if (response.userWishlist && response.userWishlist.length > 0) {
          setBookDescs(response.userWishlist);
          setDataLoaded(true);
        } else {
          // the wishlist is empty if response.userWishlist is an empty array
          setDataLoaded(true);
  ...
  useEffect(() => {
    ...
    if (bookDescs.length == 0) {
      setWishlistIsEmpty(true);
    }
    else {
      setWishlistIsEmpty(false);
    }
  }, [JSON.stringify(bookDescs)])
  ...
  return (
    <section className="min-h-screen text-gray-400 body-font">
      ...
      {wishlistIsEmpty ? (
        <div className="mt-24">
          <NoResults notFoundText="Your wishlist is empty, go add some books!" />
        </div>
      ) : (
        <Fade bottom cascade>
